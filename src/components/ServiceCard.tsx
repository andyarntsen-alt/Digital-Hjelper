...
export default function ServiceCard({ title, description, icon, href, color, featured, compact }: ServiceCardProps) {
  ...
  return (
    <Link href={href} className="no-underline group">
      <div className="bg-white rounded-lg p-5 h-full transition-colors duration-200 cursor-pointer border border-gray-200 hover:border-gray-300 hover:bg-gray-50">
        <div className="flex items-start gap-4">
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${color}`}>
            {icon}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-gray-900 mb-1 group-hover:text-nav-blue transition-colors">
              {title}
            </h3>
